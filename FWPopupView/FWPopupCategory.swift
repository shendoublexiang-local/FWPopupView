...
let fwReferenceCountKey: UnsafeRawPointer! = UnsafeRawPointer.init(
    bitPattern: "fwReferenceCountKey".hashValue
)

let fwBackgroundViewKey: UnsafeRawPointer! = UnsafeRawPointer.init(
    bitPattern: "fwBackgroundViewKey".hashValue
)
let fwBackgroundViewColorKey: UnsafeRawPointer! = UnsafeRawPointer.init(
    bitPattern: "fwBackgroundViewColorKey".hashValue
)
let fwBackgroundAnimatingKey: UnsafeRawPointer! = UnsafeRawPointer.init(
    bitPattern: "fwBackgroundAnimatingKey".hashValue
)
let fwAnimationDurationKey: UnsafeRawPointer! = UnsafeRawPointer.init(
    bitPattern: "fwAnimationDurationKey".hashValue
)
...
extension UIView {
    ...
    var fwBackgroundAnimating: Bool {
        get {
            let isAnimating =
                objc_getAssociatedObject(self, fwBackgroundAnimatingKey)
                as? Bool
    ...
    var fwAnimationDuration: TimeInterval {
        get {
            let duration =
                objc_getAssociatedObject(self, fwAnimationDurationKey)
                as? TimeInterval
    ...
    var fwReferenceCount: Int {
        get {
            let count =
                objc_getAssociatedObject(self, fwReferenceCountKey) as? Int
    ...
    var fwMaskViewColor: UIColor {
        get {
            let color =
                objc_getAssociatedObject(self, fwBackgroundViewColorKey)
                as? UIColor
    ...
    var fwMaskView: UIView {
        var tmpView =
            objc_getAssociatedObject(self, fwBackgroundViewKey) as? UIView
        if tmpView == nil {
    ...
    func showFwBackground() {
        ...
        UIView.animate(
            withDuration: self.fwAnimationDuration,
            delay: 0,
