...
open class FWAlertView: FWPopupView {
    ...
    @objc open class func alert(
        ...
        let items = [
            FWPopupItem(
                title: FWAlertViewProperty().defaultTextOK,
                itemType: .normal,
                isCancel: false,
                canAutoHide: true,
                itemClickedBlock: confirmBlock
            )
        ]
        return self.alert(
            title: title,
    ...
    @objc open class func alert(
        ...
        detail: String,
        confirmBlock: FWPopupItemClickedBlock? = nil,
        cancelBlock: FWPopupItemClickedBlock? = nil
    ) -> FWAlertView {

        let property = FWAlertViewProperty()
        let items = [
            FWPopupItem(
                title: property.defaultTextCancel,
                itemType: .normal,
                isCancel: true,
                canAutoHide: true,
                itemClickedBlock: cancelBlock
            ),
            FWPopupItem(
                title: property.defaultTextConfirm,
                itemType: .normal,
                isCancel: false,
                canAutoHide: true,
                itemClickedBlock: confirmBlock
...
extension FWAlertView {
    ...
    private func setupUI(
        ...
        self.frame.origin.x =
            (UIScreen.main.bounds.width - property.alertViewWidth) / 2
        ...
        } else if self.commponenetCount == 2
            && (detail != nil && !detail!.isEmpty)
        {
            firstTopMargin =
                property.topBottomMargin + property.commponentMargin
        } else {
        ...
        if title != nil && !title!.isEmpty {
            ...
            self.titleLabel?.snp.makeConstraints({ (make) in
                ...
                make.left.equalToSuperview().offset(
                    self.vProperty.letfRigthMargin
                )
            ...
            self.titleLabel?.textColor = UIColor.fw_colorWithStyleColors(
                lightColor: self.vProperty.titleColor,
                darkColor: self.vProperty.dark_titleColor
            )
        ...
        if detail != nil && !detail!.isEmpty {
            ...
            self.detailLabel?.snp.makeConstraints({ (make) in
                make.top.equalTo(lastConstraintItem).offset(
                    (firstTopMargin == 0)
                        ? (lastTopBottomMargin + property.commponentMargin)
                        : firstTopMargin
                )
                make.left.equalToSuperview().offset(
                    self.vProperty.letfRigthMargin
                ...
                make.height.lessThanOrEqualTo(
                    self.superview!.frame.size.height
                        * property.popupViewMaxHeightRate - 100
                )
            })
            firstTopMargin = 0
            self.detailLabel?.text = detail
            self.detailLabel?.textColor = UIColor.fw_colorWithStyleColors(
                lightColor: property.detailColor,
                darkColor: property.dark_detailColor
            )
        ...
        if inputPlaceholder != nil {
            ...
            firstTopMargin = 0
            self.inputTF?.attributedPlaceholder = NSAttributedString(
                string: inputPlaceholder!,
                attributes: [
                    NSAttributedString.Key.foregroundColor: property
                        .inputPlaceholderColor
                ]
            )
            self.inputTF?.textColor = UIColor.fw_colorWithStyleColors(
                lightColor: property.inputTextColor,
                darkColor: property.dark_inputTextColor
            )
            self.inputTF?.textAlignment = .center
            self.inputTF?.clearButtonMode = .whileEditing
            self.inputTF?.leftViewMode = .always
            self.inputTF?.layer.borderColor =
            ...
            for popupItem: FWPopupItem in items {
                ...
                if items.count == 1 {
                    btn.snp.makeConstraints { (make) in
                        make.top.left.right.equalTo(btnContrainerView).inset(
                            UIEdgeInsets(
                                top: 0,
                                left: -self.vProperty.splitWidth,
                                bottom: 0,
                                right: -self.vProperty.splitWidth
                            )
                        )
                        make.height.equalTo(
                            property.buttonHeight + property.splitWidth
                        )
                    }
                } else if items.count == 2 {
                    let btnW =
                        (property.alertViewWidth + self.vProperty.splitWidth * 4)
                        / 2
                    btn.snp.makeConstraints { (make) in
                        make.top.equalToSuperview()
                        make.left.equalToSuperview().offset(
                            -property.splitWidth + btnW * CGFloat(tmpIndex)
                        )
                    ...
                    btn.snp.makeConstraints { (make) in
                        make.left.right.equalTo(btnContrainerView).inset(
                            UIEdgeInsets(
                                top: 0,
                                left: -self.vProperty.splitWidth,
                                bottom: 0,
                        ...
                        make.width.equalTo(btnContrainerView).offset(
                            property.splitWidth * 2
                        )
                        if tmpIndex == 0 {
                            make.top.equalToSuperview()
                            lastBtn = btn
                        } else {
                            make.top.equalTo(lastBtn.snp.bottom).offset(
                                -self.vProperty.splitWidth
                            )
                            lastBtn = btn
                        }
                    }
                ...
                if popupItem.itemBackgroundColor != nil {
                    btn.backgroundColor = UIColor.fw_colorWithStyleColors(
                        lightColor: popupItem.itemBackgroundColor,
                        darkColor: popupItem.dark_itemBackgroundColor
                    )
                } else {
                    btn.backgroundColor = UIColor.fw_colorWithStyleColors(
                ...
                // 按钮文字颜色
                let titleLightColor =
                    (popupItem.itemTitleColor != nil)
                    ? popupItem.itemTitleColor
                    : ((popupItem.highlight == true)
                        ? self.vProperty.itemHighlightColor
                        : self.vProperty.itemNormalColor)
                let titleDarkColor =
                    (popupItem.dark_itemTitleColor != nil)
                    ? popupItem.dark_itemTitleColor
                    : ((popupItem.highlight == true)
                        ? self.vProperty.dark_itemHighlightColor
                        : self.vProperty.dark_itemNormalColor)
                btn.setTitleColor(
                    UIColor.fw_colorWithStyleColors(
                ...
                btn.setBackgroundImage(
                    self.getImageWithColor(color: btn.backgroundColor!),
                    for: .normal
                )
                btn.setBackgroundImage(
                    self.getImageWithColor(
                        color: UIColor.fw_colorWithStyleColors(
                            lightColor: self.vProperty.itemPressedColor,
                            darkColor: self.vProperty.dark_itemPressedColor
                        )
                    ),
                    for: .highlighted
                )
    ...
    /// 响应系统外观变化（如深色/浅色模式切换）
    /// 需要重新设置按钮的背景图片和边框颜色，因为颜色对象在不同模式下会有不同的表现
    open override func traitCollectionDidChange(
        ...
        if #available(iOS 13.0, *),
            FWPopupSWindow.sharedInstance.compatibleDarkStyle == true
        {
            if self.traitCollection.hasDifferentColorAppearance(
                comparedTo: previousTraitCollection
            ) {
                // 安全检查：确保按钮容器视图已初始化
                // 如果 items 为空，btnContrainerView 不会被创建，此时需要提前返回
                guard let btnContrainerView = self.btnContrainerView else {
                    return
                }

                for tmpView: UIView in btnContrainerView.subviews {
                    // 安全的类型转换和可选绑定，避免因类型不匹配或颜色为 nil 导致崩溃
                    guard let btn = tmpView as? UIButton,
                        let bgColor = btn.backgroundColor
                    else {
                        continue
                    }

                    // 重新设置按钮背景图片，确保在新的外观模式下显示正确
                    btn.setBackgroundImage(
                        self.getImageWithColor(color: bgColor),
                        for: .normal
                    )

                    // 根据当前外观模式设置边框颜色和高亮状态背景
                    if self.traitCollection.userInterfaceStyle == .dark {
                        btn.layer.borderColor =
                            self.vProperty.dark_splitColor.cgColor
                        btn.setBackgroundImage(
                            self.getImageWithColor(
                                color: self.vProperty.dark_itemPressedColor
                            ),
                            for: .highlighted
                        )
                    } else {
                        btn.layer.borderColor =
                            self.vProperty.splitColor.cgColor
                        btn.setBackgroundImage(
                            self.getImageWithColor(
                                color: self.vProperty.itemPressedColor
                            ),
                            for: .highlighted
                        )
                    }
...
extension FWAlertView {
    ...
    @objc private func btnAction(_ sender: Any) {
        ...
        guard let btn = sender as? UIButton,
            btn.tag < self.actionItemArray.count
        else {
            return
        }
...
open class FWAlertViewProperty: FWPopupViewProperty {
    ...
    // 深色模式：描述文字颜色
    @objc open var dark_detailColor: UIColor = kPV_RGBA(
        r: 149,
        g: 149,
        b: 149,
        a: 1
    ...
    // 深色模式：输入框文字颜色
    @objc open var dark_inputTextColor: UIColor = kPV_RGBA(
        r: 149,
        g: 149,
        b: 149,
        a: 1
