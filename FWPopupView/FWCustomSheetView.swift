...
class FWCustomSheetViewTableViewCell: UITableViewCell {
    ...
    func setupContent(
        ...
        self.line.frame = CGRect(
            x: property.separatorInset.left,
            y: ceil(self.frame.height - property.splitWidth),
            width: self.frame.width - property.separatorInset.left
                - property.separatorInset.right,
            height: property.splitWidth
        )
        self.line.backgroundColor = UIColor.fw_colorWithStyleColors(
            lightColor: property.splitColor,
            darkColor: property.dark_splitColor
        )
        ...
        if title != nil && secondaryTitle != nil && !secondaryTitle!.isEmpty {
            let attributedString = NSAttributedString(
                string: title!,
                attributes: property.titleTextAttributes
            )
            let secondaryAttributedString = NSAttributedString(
                string: secondaryTitle!,
                attributes: property.secondaryTitleTextAttributes
            )

            let titleSize = (title! as NSString).size(
                withAttributes: property.titleTextAttributes
            )
            let secondaryTitleSize = (title! as NSString).size(
                withAttributes: property.secondaryTitleTextAttributes
            )

            self.titleLabel.frame = CGRect(
                x: leftMargin,
                y: (self.frame.height - property.commponentMargin / 2
                    - titleSize.height - secondaryTitleSize.height) / 2,
                width: self.frame.width - leftMargin - property.letfRigthMargin
                    * 2,
                height: titleSize.height
            )
            self.titleLabel.attributedText = attributedString

            self.secondaryTitleLabel.frame = CGRect(
                x: leftMargin,
            ...
            && (secondaryTitle == nil || secondaryTitle!.isEmpty)
        {
            let attributedString = NSAttributedString(
                string: title!,
                attributes: property.titleTextAttributes
...
open class FWCustomSheetView: FWPopupView, UITableViewDelegate,
    UITableViewDataSource
{
    ...
    ///   - property: 可设置参数
    /// - Returns: self
    @objc open class func sheet(
        headerTitle: String?,
        itemTitles: [String]?,
        itemSecondaryTitles: [String]?,
        itemImages: [UIImage]?,
        itemBlock: FWPopupItemClickedBlock? = nil,
        property: FWCustomSheetViewProperty?
    ) -> FWCustomSheetView {
        ...
        customSheet.setupUI(
            ...
            itemSecondaryTitles: itemSecondaryTitles,
...
extension FWCustomSheetView {
    ...
    private func setupUI(
        ...
        if headerTitle != nil {
            self.headerView = UIView(
                frame: CGRect(
                    ...
                    height: property.headerViewHeight
                )
            )
            self.headerView?.backgroundColor = UIColor.fw_colorWithStyleColors(
            ...
            line.frame = CGRect(
                x: 0,
                y: floor(property.headerViewHeight - property.splitWidth),
                width: selfSize.width,
                height: property.splitWidth
            )
            line.backgroundColor = UIColor.fw_colorWithStyleColors(
                lightColor: self.vProperty.splitColor,
                darkColor: self.vProperty.dark_splitColor
            )
            self.headerView!.addSubview(line)

            let titleLabel = UILabel(
                frame: CGRect(
                    x: property.letfRigthMargin,
                    y: 0,
                    width: selfSize.width - property.letfRigthMargin * 3,
                    height: property.headerViewHeight
                )
            )
            ...
            let closeBtn = UIButton(
                frame: CGRect(
                    x: selfSize.width - (property.headerViewHeight - 2)
                        - property.letfRigthMargin / 2,
                    y: 1,
                    width: property.headerViewHeight - 2,
                    height: property.headerViewHeight - 2
                )
            )
            ...
            if property.closeImage != nil {
                ...
                let url = Bundle(for: FWCustomSheetView.self).url(
                    forResource: "FWPopupView",
                    withExtension: "bundle"
                )
                if url != nil {
                    let imageBundle = Bundle(url: url!)
                    let path = imageBundle?.path(
                        forResource: "cs_close@3x",
                        ofType: "png"
                    )
                    if path != nil {
                        closeBtn.setImage(
        ...
        self.tableView.register(
            FWCustomSheetViewTableViewCell.self,
            forCellReuseIdentifier: "cellId"
        )
        self.tableView.separatorStyle = .none
        self.tableView.backgroundColor = UIColor.fw_colorWithStyleColors(
            lightColor: self.vProperty.backgroundColor,
            darkColor: self.vProperty.dark_backgroundColor
        )
        self.tableView.bounces = property.bounces

        if property.popupViewMaxHeightRate > 0
            && property.popupViewItemHeight * CGFloat(self.itemsCount())
                > property.popupViewMaxHeightRate
                * self.superview!.frame.size.height
        {
            selfSize.height =
                property.popupViewMaxHeightRate
                * self.superview!.frame.size.height
        } else if property.popupViewMinHeight > 0
            && property.popupViewItemHeight * CGFloat(self.itemsCount())
                < property.popupViewMinHeight
        {
        ...
        self.tableView.frame = CGRect(
            x: 0,
            y: property.headerViewHeight,
            width: selfSize.width,
            height: selfSize.height - property.headerViewHeight
        )

        // 用来隐藏多余的线条，不想自定义线条
        let footerView = UIView(
            frame: CGRect(x: 0, y: 0, width: self.frame.width, height: 1)
        )
...
extension FWCustomSheetView {
    ...
    public func tableView(
        ...
        guard
            let cell = tableView.dequeueReusableCell(
                ...
                for: indexPath
            ) as? FWCustomSheetViewTableViewCell,
            let property = self.vProperty as? FWCustomSheetViewProperty
        else {
        ...
        // 安全访问数组，避免越界
        let title =
            (self.itemTitleArray != nil
                && indexPath.row < self.itemTitleArray!.count)
            ? self.itemTitleArray![indexPath.row] : nil
        let secondaryTitle =
            (self.itemSecondaryTitleArray != nil
                && indexPath.row < self.itemSecondaryTitleArray!.count)
            ? self.itemSecondaryTitleArray![indexPath.row] : nil
        let image =
            (self.itemImageNameArray != nil
                && indexPath.row < self.itemImageNameArray!.count)
            ? self.itemImageNameArray![indexPath.row] : nil
        ...
        cell.backgroundColor = UIColor.fw_colorWithStyleColors(
            lightColor: self.vProperty.backgroundColor,
            darkColor: self.vProperty.dark_backgroundColor
        )
        if property.lastNeedAccessoryView == true
            && indexPath.row == (self.itemsCount() - 1)
        {
    ...
    public func tableView(
        ...
        guard let property = self.vProperty as? FWCustomSheetViewProperty else {
            return
        }
...
open class FWCustomSheetViewProperty: FWPopupViewProperty {
    ...
    /// 副标题字体属性
    @objc public var secondaryTitleTextAttributes:
        [NSAttributedString.Key: Any]!

    /// 内容位置
    @objc public var contentHorizontalAlignment:
    ...
    public override func reSetParams() {
        ...
        self.titleTextAttributes = [
            NSAttributedString.Key.foregroundColor:
                UIColor.fw_colorWithStyleColors(
                    lightColor: self.itemNormalColor,
                    darkColor: self.dark_itemNormalColor
                ), NSAttributedString.Key.backgroundColor: UIColor.clear,
            NSAttributedString.Key.font: self.buttonFont,
        ]

        let tmpColor = kPV_RGBA(r: 138, g: 146, b: 165, a: 1)
        self.secondaryTitleTextAttributes = [
            NSAttributedString.Key.foregroundColor: tmpColor,
            NSAttributedString.Key.backgroundColor: UIColor.clear,
